...
function createApp({ yesPhrases, noPhrases }) {
  ...
  app.get('/', (req, res) => {
    ...
    const selectedKind = VALID_YES_KINDS.has(kind) ? kind : 'any';
    const filtered = selectedKind === 'any' ? yesPhrases : yesPhrases.filter((item) => item.kind === selectedKind);
    return res.render('index', { phrase: pickRandom(filtered).text, selectedKind });
  });
